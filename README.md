...
* **Pitch Deck Video** — problem overview, solution architecture, and strategic impact.
* **App Walkthrough** — real-time verification, liveness tests, and admin geofencing configuration in action.
* **Slides Deck:** [NHAI Hackathon 7.0 Pitch Presentation](./NHAI_Project_Presentation.pptx)
...
[ Inside Site Geofence ]                              [ Outside Geofence ]
       │                                                       │
       ▼                                                       ▼
[ RNCamera Initialized ]                             [ ERROR: Block Action ]
       │
       ▼
...
[ Liveness State Machine (Blink Validation) ]
 └─► waiting_open ──► eyes_open ──► eyes_closed ──► blink_confirmed
                                                               │
                                                               ▼
...
* The state machine guides the worker: *Open Eyes* ➔ *Close Eyes* (EAR drops below 0.25) ➔ *Open Eyes* (EAR rises above 0.70).
...
1. Clone the repository:
   ```bash
   git clone https://github.com/Sahil-Chandel/NHAI-Hackthon-7.0.git
   cd nhai_prj
   ```
...
All metrics verified on entry-level Android devices (e.g., Redmi A-series):

* **Face Detection**: 120ms - 250ms
* **Liveness Detection Loop**: 2.5s - 4.5s (dependent on blink speed)
* **Mathematical Vector Matching**: <15ms
* **Local Database Transaction**: <5ms
* **Total End-to-End Authentication**: **3.5 - 6 seconds**
* **Memory Footprint**: <80MB RAM usage

---

**Built with Love for NHAI Hackathon 7.0**
*Offline-First • Privacy by Design • Built for India's Infrastructure* 🇮🇳

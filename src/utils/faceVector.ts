...
export const computeFaceVector = (face: any): number[] | null => {
  ...
  for (const r of scaled) vector.push(r);
  ...
  const mouthAngle = Math.atan2(rightMouth.y - leftMouth.y, rightMouth.x - leftMouth.x);
  const noseToMouthAngle = Math.atan2(mouthCenter.y - nose.y, mouthCenter.x - nose.x);
  const leftEyeToMouthAngle = Math.atan2(leftMouth.y - leftEye.y, leftMouth.x - leftEye.x);
  const rightEyeToMouthAngle = Math.atan2(rightMouth.y - rightEye.y, rightMouth.x - rightEye.x);
  vector.push(
    eyeAngle * 3,
    mouthAngle * 3,
    noseToMouthAngle * 3,
  ...
  const { origin, size } = face.bounds || { origin: { x: 0, y: 0 }, size: { width: 1, height: 1 } };
  ...
  const asymmetryMouth = Math.abs(d_leftEye_leftMouth - d_rightEye_rightMouth) / d_eyes;
